...
int main(int argc, char *argv[])
{
    try
    {
        std::ofstream file("bandwidth_matmul_i8.csv", std::ios::app);
        ...
        ov::Shape input_shape_A{1, size};
        ov::Shape input_shape_B{size, 1};
        ...
        auto matmul_op = std::make_shared<ov::op::v0::MatMul>(quantized_A, quantized_B, false, false);
        auto convert_result_op = std::make_shared<ov::op::v0::Convert>(matmul_op, ov::element::f16);
        auto result = std::make_shared<ov::op::v0::Result>(convert_result_op);
        ...
        std::cout << infer_request.get_input_tensor(1).get_element_type() << std::endl;
        std::cout << infer_request.get_input_tensor(1).get_shape() << std:: endl;
        std::cout << infer_request.get_input_tensor(1).get_byte_size() << std::endl;

        std::vector<float> input_data_A(size, 1.0f);
        std::vector<float> input_data_B(size, 1.0f);

        // input_data_A[size * 0 + 0] = 1.0f;
        // input_data_B[size * 0 + 0] = 2.0f;
        ...
        file << infer_request.get_input_tensor(0).get_byte_size() +infer_request.get_input_tensor(1).get_byte_size() << ",";
