...
int main(int argc, char *argv[])
{
    try
    {
        std::ofstream file("power_matmul_by_mul-RS_i32.csv", std::ios::app);
        file << std::endl;
        file << argv[1] << ",";
        file << argv[2] << ",";
        file << argv[3] << ",";
        ...
        size_t looploop = std::stoi(argv[3]);
        ...
        auto input_B = std::make_shared<ov::op::v0::Parameter>(ov::element::i32, input_shape_B);

        auto multiply_op = std::make_shared<ov::op::v1::Multiply>(input_A, input_B);
        ...
        std::vector<int32_t> input_data_A(size * size, 1);
        std::vector<int32_t> input_data_B(size * size, 1);

        input_data_A[size * 0 + 0] = 2;
        // input_data_B[size * 0 + 0] = 1;
        ...
        for (size_t loop = 0; loop < looploop; ++loop)
        {
            infer_request.set_tensor(input_A, ov::Tensor(ov::element::i32, input_shape_A, input_data_A.data()));
            infer_request.set_tensor(input_B, ov::Tensor(ov::element::i32, input_shape_B, input_data_B.data()));
